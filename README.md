...
8. **Furnishing.** Cloisters with arcades around lawns and quatrefoil pools. Hexagonal fountains, long basins with a row of jets, the golden sun-idol on its inscribed plinth, obelisks, a gilded armillary sphere, domed pavilions, great pointed gateways, horseshoe arches with red and cream voussoirs, arcades along the drops, planters of agave and broad leaves, palm orchards, cypresses in stone curbs, lamp standards, braziers, banner poles, carved steles, urns and benches. Some plazas get a **rill**: a narrow channel fed by a spout in a wall, crossed by stepping slabs at every cell so it never cuts the terrace in two. At a drop it spills through a scupper into a basin or canal below; otherwise it ends in a small basin with a jet. Walls rising from a terrace may carry a carved relief (devotees bowing to the sun, as a panel or a lunette), a wall fountain pouring into a half-round basin, or a **pergola**: columns out on the terrace carrying timber joists and a vine, with benches in its shade. Terraces that no path reaches sink into deep courts (or, in wet districts, flooded wells fed by culverts), or where the ground rises stay up as hanging gardens, planted as orchards in rows. A sunken parterre in a wet district is a **reflecting pool** instead, its water just below the paving: a long one gets rows of jets arching in from both sides, a squarish one an islet with a plume. Roofs are dressed too: a **chhatri** (a little open kiosk under a dome) on each corner of a flat roof or round a dome, a pair of slotted wind catchers, palms in pots set out square, or a turret.
9. **Composition.** Furnishing is laid out, not scattered (`src/world/compose.js`). Each plot knows its ways in: the heads and feet of its stairs, gates from the next block and the ends of bridges. A plaza takes its **axis** from the entrance that looks best across it, and its centrepiece stands on that line, halfway across, facing whoever comes in (a grand arch or a horseshoe arch turns so you walk through it). A **paved runner** of cream slabs may lead from the entrance up to it, and from a second entrance straight across. Either side of the axis, **allées** of cypresses, palms in curbs, lamps or steles stand in mirrored pairs, spaced evenly about the middle; otherwise four matching lamps, braziers or cypresses mark the corners round the centrepiece, or two benches face it across the axis. A long narrow plaza gets a single row down its middle instead, like the median of a boulevard. Each plot picks one kind of frame for its ways in (lamps, braziers, banners, cypresses, steles) and sets a matching pair either side of every stair head, gate and bridge end: banners and braziers climb the ziggurats, braziers burn on their summits, lamps light the quays in step on both sides of a canal. Whatever else stands about a big plaza comes in pairs mirrored across its axis. Corners get one treatment per plot: matching urns, palms or cypresses in the inner corners, and pinnacle piers or chhatris to look out from on the outer corners over the drops. Doors may sit in a **portal** (striped voussoirs, a cream spandrel in a raised frame, an inscribed band) or between a pair of lanterns on brackets, with the wall beside them left plain and sometimes a bench under each. Composed pieces keep a cell apart from one another and from the planters along the walls, so no sliver of floor gets shut in between them.
10. **Walls.** Every retaining wall is a facade. It gets rows of pointed, twin, tall or blind-arched windows, carved friezes, balconies on corbels, cornices, string courses and doors (teal leaves with red diamonds). The pattern is chosen per wall and per storey, so each facade stays consistent.
...
- a soft bloom around foam and jets, whose mask rides in the colour target's alpha, and flickering flames in the lamps and braziers
